...
esp_err_t config_svc_init(void)
{
    ...
    uint32_t tmp;
    s_cfg.max_voltage_01v = (hal_nvs_get_u32(NVS_KEY_MAX_V, &tmp) == ESP_OK)
                            ? (uint16_t)tmp : DEFAULT_MAX_V;
    s_cfg.max_current_01a = (hal_nvs_get_u32(NVS_KEY_MAX_A, &tmp) == ESP_OK)
                            ? (uint16_t)tmp : DEFAULT_MAX_A;

    int32_t soc;
    s_cfg.target_soc = (hal_nvs_get_i32(NVS_KEY_SOC, &soc) == ESP_OK)
                       ? (int8_t)soc : DEFAULT_SOC;

    if (hal_nvs_get_str(NVS_KEY_SSID, s_cfg.wifi_ssid, sizeof(s_cfg.wifi_ssid)) != ESP_OK) {
        s_cfg.wifi_ssid[0] = '\0';
    }
    if (hal_nvs_get_str(NVS_KEY_PASS, s_cfg.wifi_pass, sizeof(s_cfg.wifi_pass)) != ESP_OK) {
        s_cfg.wifi_pass[0] = '\0';
    }

    bool beacon;
    s_cfg.beacon_unlocked = (hal_nvs_get_bool(NVS_KEY_BEACON, &beacon) == ESP_OK) && beacon;
...
esp_err_t config_svc_set_charging(uint16_t v_01v, uint16_t a_01a, int8_t soc)
{
    ...
    esp_err_t r = hal_nvs_set_u32(NVS_KEY_MAX_V, v_01v);
    r |= hal_nvs_set_u32(NVS_KEY_MAX_A, a_01a);
    r |= hal_nvs_set_i32(NVS_KEY_SOC,  soc);
    return r;
}
...
esp_err_t config_svc_set_wifi(const char *ssid, const char *pass)
{
    ...
    esp_err_t r = hal_nvs_set_str(NVS_KEY_SSID, s_cfg.wifi_ssid);
    r |= hal_nvs_set_str(NVS_KEY_PASS, s_cfg.wifi_pass);
    return r;
}
...
esp_err_t config_svc_set_beacon(bool unlocked)
{
    ...
    return hal_nvs_set_bool(NVS_KEY_BEACON, unlocked);
}
